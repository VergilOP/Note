...
if [ "$1" = "" ]
then
    echo "You forgot to add the assignment name, e.g. 'Homework1'."
...
if ghc --version | grep 'version 8.10.7' > /dev/null; then
    echo 'Correct version of GHC found.'
else
    echo "Unexpected version of GHC found; expected 8.10.7."
    echo "Please ensure you are on JupyterLab with the right settings."
    exit 1
fi
...
ghc $1.hs -package mtl -odir $temp_dir -hidir $temp_dir
...
then
    ...
    echo -e "\t(i) your file is called something like 'HomeworkX.hs'"
    echo -e "\t(ii) you did not change the top of the template"
    echo "and try again."
    exit 1
fi

ghc -XSafe $1.hs -package mtl -odir $temp_dir -hidir $temp_dir
