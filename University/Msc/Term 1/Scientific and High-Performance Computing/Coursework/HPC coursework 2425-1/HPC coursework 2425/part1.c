...
void init(double u[N][N], double v[N][N]){
    #pragma omp parallel  		// Parallelize nested loops
	{
		for (int i=0; i < N; i++){
			for (int j=0; j < N; j++){
...
void dxdt(double du[N][N], double dv[N][N], double u[N][N], double v[N][N]){
	...
	#pragma omp parallel private(lapu, lapv, up, down, left, right)  	//Protect the private variables
	{
		for (int i = 0; i < N; i++){
			for (int j = 0; j < N; j++){
				if (i == 0){
...
double norm(double x[N][N]){
	double nrmx = 0.0;
    #pragma omp parallel reduction(+:nrmx)
	{
		for (int i = 0; i < N; i++){
			for (int j = 0; j < N; j++){
				nrmx += x[i][j]*x[i][j];
